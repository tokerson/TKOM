...
//        assertEquals(Tokens.OPERATORS.get().toString(), token.getTokenType().toString());
    ...
    @Test
    public void isIdentifiyingBackslash(){
        String id = "\\";
        Token token = getTokenFromString(id);
        assertEquals("token's content should be \\","\\", token.getContent());
        assertEquals("token's type should be BACKSLASH_SYMBOL",TokenType.BACKSLASH_SYMBOL, token.getTokenType());
    }

    @Test
    public void isIdentifiyingQuoteSymbolInsideString(){
        String id = "\\\"";
        Token token = getTokenFromString(id);
        assertEquals("token's content should be \\\"","\\\"", token.getContent());
        assertEquals("token's type should be QUOTE_SYMBOL",TokenType.QUOTE_SYMBOL, token.getTokenType());
    }
